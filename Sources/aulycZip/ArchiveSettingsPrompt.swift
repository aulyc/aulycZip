...
struct EncryptedArchiveCreationChoice: Sendable {
    let password: String
...
@MainActor
enum ArchiveSettingsPrompt {
    static func requestEncryptedArchiveChoice(
        for initialRequest: EncryptedArchiveRequest
    ) -> EncryptedArchiveCreationChoice? {
        let password = secureTextField(
        ...
        let outputFileName = initialRequest.destinationURL
            .deletingPathExtension()
            .lastPathComponent
        let locationController = EncryptedArchiveLocationController(
        ...
        let accessory = encryptedArchiveAccessory(
            locationController: locationController,
            sourceRequest: initialRequest,
            password: password,
            confirmation: confirmation
        )
        var validatedRequest: EncryptedArchiveRequest?
        let dialog = ArchiveActionDialog(
            accessoryView: accessory,
            supplementaryButton: randomPasswordButton,
            primaryButtonTitle: "创建",
            initialFirstResponder: password,
            validationHandler: {
                ...
                do {
                    validatedRequest = try EncryptedArchiveRequest(
        ...
        guard dialog.runModal() == .alertFirstButtonReturn,
              let validatedRequest
        else { return nil }
        return EncryptedArchiveCreationChoice(
    ...
    private static func encryptedArchiveAccessory(
        locationController: EncryptedArchiveLocationController,
        sourceRequest: EncryptedArchiveRequest,
    ...
    private static func encryptedArchiveHeader() -> NSView {
        appDialogHeader(
            title: "设置文件加密压缩密码",
            accessibilityIdentifier: "encrypted-archive-settings-title"
        )
    }
