...
- 在 Finder 右键菜单中加密压缩所选文件或文件夹，或直接解压单个 ZIP
...
Finder 中选中单个 ZIP 后，右键选择“使用 aulycZip 解压”，会进入与菜单栏相同的解压设置和安全校验流程。多个 ZIP 需要分别解压。

新密码至少需要 8 个字符，不是只能输入 8 位，也不要求必须是数字。建议使用 12 个以上字符，并混合字母、数字和符号。

菜单栏与 Finder 右键均提供加密 ZIP 创建与 ZIP 解压。普通 ZIP 创建交给 macOS Finder 自带的“压缩”功能。
